...
function start()
{
    ...
    db.transaction(populateDBTest, errorCB, successCB);
...
function populateDBTest(tx)
{
    ...
    tx.executeSql("CREATE TABLE IF NOT EXISTS pictures (picture_id  INTEGER PRIMARY KEY AUTOINCREMENT,picture_name  TEXT,picture_url TEXT)");
    tx.executeSql("CREATE TABLE IF NOT EXISTS categories (category_id INTEGER PRIMARY KEY AUTOINCREMENT,category_name TEXT)");

    tx.executeSql("CREATE TABLE IF NOT EXISTS tags (tag_id  INTEGER PRIMARY KEY AUTOINCREMENT,tag_name TEXT)");
